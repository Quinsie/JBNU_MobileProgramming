# backend/source/ai/buildSecondReplayParquetStart.py
...
def build_second_replay_parquet(target_date, mean_date):

    print(f"[INFO] 시작: {target_date} replay 전처리 [Second Model]")

    start_date = datetime.datetime.strptime("20250505", "%Y%m%d")
    end_date = datetime.datetime.strptime(target_date, "%Y%m%d") - datetime.timedelta(days=1)
...
if __name__ == "__main__":
    ...
    mean_date = (datetime.datetime.strptime(target_date, "%Y%m%d") - datetime.timedelta(days=2)).strftime("%Y%m%d")
    build_second_replay_parquet(target_date, mean_date)
