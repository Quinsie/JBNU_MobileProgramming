...
FEATURE_PARQUET_PATH = os.path.join(BASE_DIR, "data", "preprocess", "first_train", "self_review")
...
def extract_route_info(stdid, stdid_number, label_bus):
    route_str = stdid_number[str(stdid)]  # e.g. "1001A1"
    bus_number_str = route_str[:-2]       # "1001"
...
if __name__ == "__main__":
    ...
    for fname in os.listdir(FORECAST_DIR):
        if fname.endswith(".json"):
            forecast_all[fname.replace(".json", "")] = load_json(os.path.join(FORECAST_DIR, fname))
        ...
        for stdid in entry['stdid']:
            ...
            for stop in stops:
                ...
                for k, v in row.items():
                    key = k.replace("x_", "")
                    x_tensor[key] = torch.tensor([[v]], dtype=torch.long if isinstance(v, int) else torch.float32)
            ...
            eta_result[f"{stdid}_{hhmm}"] = eta_list

    # 저장
    with open(os.path.join(ETA_SAVE_PATH, f"{date_str}.json"), "w", encoding="utf-8") as f:
        json.dump(eta_result, f, ensure_ascii=False, indent=2)

    df = pd.DataFrame(parquet_rows)
    df.to_parquet(os.path.join(FEATURE_PARQUET_PATH, f"{date_str}.parquet"), index=False)
