...
#### 진행 상황
 ...
 - 종속성 문제까지 끝냈고, 이제 실험 시작.
 - 실험 비교군은 prev mean elapsed(직전 정류장까지 평균 도착시간 4종)를 포함한 모델(_2)과, 기존 모델이다.
 - 5/7부터 320개 dummy row data를 포함한 상태로 self-review까지 loop를 돌릴 것이며, 5/22 데이터까지 뽑고 난 뒤 검증을 수행한다. (measure Error)
 - 여러 표본 검증 방법(분산/표준편차/최대/최소나 여러가지 등등.)을 사용하여 상호 비교를 할 것이다. 큰 차이가 없다면 pme를 사용하는 쪽으로 가지 않을까?
 - 이거 끝나고 나면 더 나은걸로 통일해서, 일주일치 한번에 학습 + 진행한 뒤 결과랑, 더미 320개 넣고 하루단위 학습 결과랑 비교해서 더 나은쪽으로 확정하겠다.
 - 이번 주말은 이걸로 1차 모델 완성하는 방향으로.

---
### 2025-05-24
#### 진행 상황
 - 우선 사용 가능한 데이터 수준까지 학습 후 결과로 나온 eta table 분석을 통해 어떤 model이 가장 괜찮은지 실험을 해볼 예정이다.
 - pme 포함 비교군(2번)과 기존 모델(1번)간 결과가 우수한 모델로 좁힌다.
 - 이후 우수한 모델로 일주일간 데이터로 첫 학습 후 학습을 쌓아나가는 모델과 기존 모델 간 결과가 우수한 모델로 최종 결정한다.
 - 추론 결과 파악을 위해 뽑아볼것들을 생각해봐야겠다.
 ...
 - replayparquet 날씨 전처리기 음수 fallback 수정했다.
 - 제대로 들어가기는 하는건지 궁금해서 확인해보려고 했는데, 실험 비교군이었던 pme 모델 코드에 prev ord ratio 없이 그냥 pme와 me를 concat한 흔적을 발견했다.
 - 모델 입장에서 "과거 정보 + 과거 평균"이 아닌, 그냥 과거 평균과 현재 평균을 수치적으로만 얻어서 뭐가 과거인지 알 수가 없는 상황이 됐던 것.
 - 다시 학습-추론한다. 23일것까지 추론 돌리고 평가하기로 하자.

---
### 2025-05-25
#### 진행 상황
 - 갑자기 본가에 오게 돼서 신경을 전혀 못써줬다.
 - 오늘 하루종일 또 살펴보고있을 수가 없을 것 같아 일단 자동으로 돌려주는 스크립트를 짰다.
 - 결과 나오면 분석 후에 새로운 실험 바로 시작하면 될 듯 하다. 새로운 실험까지 끝나면, 성능지표 분석 최종으로 한 뒤 1차모델은 확정하도록 하겠다.
 - 3시간 30분간 5/24 추론까지 완료. 이제 성능지표 분석을 시작한다.

#### 노트
 - 성능 지표 (Metrics)
  + MAE(Mean Absolute Error): 모든 오차의 절댓값 평균. 예측이 평균적으로 얼마나 틀렸는지 (단위: 초). 직관적이며 아웃라이어 영향 적음
  + RMSE(Root Mean Squared Error): 제곱 오차 평균의 루트. MAE보다 아웃라이어에 민감. 예측 오차가 클수록 더 강하게 패널티.
  + MAPE(Mean Absolute Percentage Error):	실제값 대비 예측 오차 비율의 평균 (%)	상대적 오차. 경로가 짧은 경우에도 오차가 큰지 평가 가능. 단, 실제값이 0에 가까우면 불안정.
  + Max Error:	가장 큰 예측 오차. 최악의 사례 확인용. 예측 시스템의 신뢰성 한계 파악 가능.
  + Percentile Errors(25%, 50%, 75%, 95%): 오차 분포의 분위수. 50%는 median error, 95%는 대부분의 예측 정확도를 대표함. 이상치 영향 없이 분포 파악 가능.
  + SDE(Standard Deviation of Errors): 오차의 표준편차. 예측 오차의 변동성(일관성)을 나타냄. 작을수록 안정적인 모델.

---
### 2025-05-26
#### 진행 상황
 - 아. 데이터 무결성을 인정하려고 했는데, 자정 기점으로 데이터에 이상이 조금 있고 얘가 지금 학습흐름을 좀 망치고 있었던 느낌이다.
 - 자정 기준 버스 로그 저장 로직을 확실히 해서 적용시켜야겠다. 우선 오늘 1차 모델 확정하려고 했는데, 좀 문제가 생겨서 힘들듯;
 - 코드를 다시 뜯어보는데, 논리적으로 지금 말이 안 되는 상황이 일어난다. 코드상으로는 절대로 벌어지면 안 되는 자정 문제 버그다.
 - 도저히 여러번을 살펴봐도 정말 말이 안 된다. 모든 경우에 대해 timeout을 시켜놨는데. 이러면 전주시 BIS를 의심해야한다.
 - 일단 API쪽 문제라고 가정하고, 7200-sec-timeout logic을 삽입해서 급한 불이라도 꺼야겠다. 이게 무슨 황당한 일인가.
 - 데이터 무결함을 의심하고싶진 않았는데 이렇게 되면 답이 없다. 의심하는수밖에.
 - 매일 학습 이전, 새롭게 데이터를 한번 가공을 해주긴 해야겠다. 지금 보니까 실제 삭제되는 log들이 꽤 존재한다.
...
prev_pred_interval (self-review용, 이전에 예측했던 각 ord에 대한 소요시간)
 ...
 - 음... 2차 모델을 만들긴 했는데(학습코드까지), 이게 성능이 잘 나올지 감이 안 잡힌다.
 - LSTM 전환 가능성도 염두에 두면서 해야할듯... 솔직히 잘 모르겠다. 뭔가 점점 자신이 없어진다.
 - 열심히 열과 성의를 다했는데 MLP기반 2차가 생각보다 굉장히 feature 숫자도 적고 허술하며, 때려맞추기 식이 되는거같아서 기분이 나쁘다.
 - 우선 빠르게 해보자. 성능이 안 나오면 LSTM 전환도 생각하고, 5초 단위로 로그를 끊어 받는거까지 생각해야한다.
 - 다음 task로는... route_node 단위로 각 ord(+10 확장? +5? 이건 고민)에 대한 elapsed feature 산출이다.
 - 이게 끝나면 파케이 생성기 replay먼저 만들고, 추론기 만들고, 다시 review 파케이 생성기까지 만들면 끝...
 - 이번주안에 끝내보자. 이번주 일도 많아서 좀 많이 바쁘다.

---
### 2025-05-28
#### 진행 상황
 - 와우. 실시간성을 도입하려니 생각보다 일이 많이 복잡해졌다. 우선 현재까지의 방향은:
  + 1. 추론기는 API 호출에 의한 응답만 해준다.
  + 2. 실시간 중계기에서 trackSingleBus로부터 route_node가 갱신될 때마다 정보를 받고, 그걸 추론기로 보내 응답을 받는다.
  + 3. 해당 응답을 기반으로 실시간 ETA Table에 갱신한다.
  + 4. 반성기(Self-Review parquet 생성기)는 실시간 중계기에서 한 방법과 동일하게 추론기에 요청 -> 응답을 받아 파케이를 생성한다.
  + 5. 반성 수행 후 복습 파케이 제작 -> 복습 수행까지 마치면, 새로운 모델이 나온다.
  + 6. 해당 새로운 모델을 05시 30분 runAll.py를 통해 추론기를 재부팅하며 새로운 모델로 load한다.
  + 7. 따라서 추론기는 24시간 가까이 실행중이어야 하고, 매일 05시 30분에 새로운 모델로 load되어야 한다.

---
### 2025-05-29
#### 진행 상황
 - 급한 불을 먼저 꺼야한다. 과제가 조금 밀려서 그쪽으로 집중중임.
 - 우선 이번주까지 어떻게든 2차 모델 완성하는걸 목표로 해야한다.
 - 그래야 다음주중에 길찾기 Agent 만들면서 안드로이드 프론트엔드 제작 가능.
 - 음... 2차 모델 만들면서 유틸리티 캡슐화를 병행하고있었는데, 기존 1차 모델에 사용하던 fallbackWeather 함수가 잘못된거같다.
 - fallback을 10분 단위로, 자기 자신 시간을 기점으로 돌리는데 이것 자체가 잘못됐다.
 - 파일이 10분 단위로 이름이 생성되어있을거라는 보장을 못하는데... 그럼 지금 학습된 1차 모델은 기본값 날씨가 들어갔을 가능성이 농후하다.
 - 우선 급하게 for loop 180 times, timedelta minutes = 1로 수정했다.

---
### 2025-05-30
#### 진행 상황
 - 메모리 관점에서 최적화를 진행하다보니 엄청난 이슈가 나왔다.
 - 지금 상태면 딕셔너리를 계속 deepcopy하고있는 멍청한 상황이다. COW 공유를 제대로 적용해야한다.
 - RAM 누수 및 시간만 엄청 잡아먹는 상황이 나오고 있다.
 - 해결이 필요하다. 오늘 내로 이건 꼭 해결한다.

---
### 2025-05-31
#### 진행 상황
 - 서울 방문으로 인해 하루동안 진척이 없었다.
 - 우선 메모리 누수 문제는 해결했다. 지금부터 1차 모델 재학습 돌려놓고 2차 전처리기 만들면서 확인해볼 예정.
 - 와오... 메모리 누수뿐 아니라 그냥 전체적인 효율 자체가 늘었다. 30초 걸리던 전처리가 2초만에 끝난다;
 - 딕셔너리를 각 subprocess로 linux 환경에서 넘겨줄 때에는 무조건 init worker 사용을 해야겠다. 말도 안 되네.
 - 진짜 눈을 의심했다;

---
### 2025-06-01
#### 진행 상황
 - 2차 모델 하루치 replay 학습을 진행해봤다.
 - 추론기-복습용파케이생성기-중계기까지 전부 다 만들어야되는데 언제 해야 하는지... 잠을 더 줄여야겠다.
 - 2차 만지면서 남는 서버 노는 시간에 1차 모델 하이퍼파라미터 조정 해보면서 더 조여봐야겠다.
 - 2차는 지금 보아하니 어떤거는 3초, 어떤거는 10초, 어떤거는 40초단위 오차가 된다.
 - 구간 단위 예측... 이것도 어떻게 보면 과거 통계 기반으로 구간단위 예측을 할 뿐이라... 뭔가 조금 막막하다.
 - 제대로 될런지 모르겠다. 적어도 기존 서비스보단 성능이 잘 나와야할텐데.
 - 1차 실험을 2주일치 시작으로 조정해보려고 했는데, RAM 부족(32GB + 스왑메모리 전부 사용해도 불가능)으로 인해 학습 자체가 안된다. RAM확장을 염두에 두고 있으며, 확장한 뒤에 한번 2주일치 학습을 시도해보겠다. (2차 모델도 1주일치 해야하는거 생각하면 64GB 확장은 불가피할듯..)

---
### 2025-06-02
#### 진행 상황
 - 2차 모델 제작하는 동시에 현재 1차 모델 하이퍼파라미터 조정 실험을 지속중이다.
 - 최초 Batch Size 512 -> 256 -> 128로 갈 수록 성능 향상이 보여서, 64 -> 32까지 조여볼 생각이다.
 - 아마도 1개 노선 평균 ORD가 40~60사이에 형성되는데, 512~256은 너무 많은 노선으로 노선특징이 희석됐을것으로 보인다.
 - 만약 64에서 성능이 다시 떨어진다면, 최종 batch size는 128로 설정한 뒤 hidden layer & dimension 조정으로 가보겠다.
 - 우선 실험은 눌러놨고... trackSingleBus에서 공유 큐에 route node정보를 add하고, 그걸 수신하는 중계기 틀을 만들었다.
 - 바로 적용해봤고, 성공. 공유 큐로 문제 없이 node가 바뀔 때마다 정보를 잘 넘겨준다.
 - 다음은 추론기다. 특정한 row또는 row list를 받으면 추론을 실시하여 돌려주는 방식으로 제작해야겠다.
 - Batch 64 결과가 나왔다. 토/일요일에 대해서 아주 훌륭한 수준으로 추론이 이뤄진다. 반면에 평일은 128에 비해 조금 떨어진다.
 - 따라서 토/일/월요일 학습만 batch 64, 나머지 평일은 batch 128로 하는 dynamic batch로 마지막 실험을 해보자.
 - 시험공부 하기 싫어서 하다보니 어느새 2차 모델까지 뚝딱 완성했다.
 - 2차 모델 실시간용 중계기만 만들면 2차도 끝. 아직 테스트는 안해봤는데, 적어도 과거 정보 한정 학습루프는 완성했다.
 - 테스트 해서 작동 이상 없으면 오늘 돌리고 자면 될듯. 얘도 실험 반복하면서 성능 안정화 하자.
 - 내일모레 첫 시험 보고나서부터 길찾기 에이전트 제작하면서 + 안드로이드 프론트엔드 작업 들어가면 될 것 같다.
 - 2차 모델로 예측하는 ORD+1~5의 경우 ORD1은 못해도 10~20초 안으로 들어와야 하므로, 더 무겁게 반영했다.
 - [3.0, 2.0, 1.5, 1.2, 1.0]으로 배수 반영해서 1번은 최대한 정확하게 예측하고자 했다.

---
### 2025-06-03
#### 진행 상황
 - 2차 모델 추론치가 생각보다 이해가 안 가는 부분이 있어서 파케이를 뜯어보니, 내가 함수를 잘못 만져서 "교통흐름"이 ORD+2부터 0이었다.
 - 저거 수정 했고... 대충 상황을 보아하니 2차 모델 학습시간이 좀 압도적으로 길다.
 - RAM 먹는 문제는 해결했다. 64GB를 달아도 전부 다 쓰고 + 스왑메모리 12GB까지 먹는걸 보고 황당했는데, 그냥 내가 잘못 코딩한거였다.
 - list로 만들고 섞고 하는 과정에서 불필요하게 많이 복사된게 원인이었고, 전부 해결하고 나니 10GB 미만으로 먹는다.
 - 굳이 집에서 남는 RAM을 들고와서 꽂을 이유가 없었다 (...)
...
### 2025-06-04
#### 진행 상황
 - 중계기와 전송기도 만든다. 전송기는 중간서버로 API POST를 하는 전용 모듈이다.
  + 1. n개(1~수십개) trackSingleBus가 online, 버스가 새로운 route_node에 접근하거나, 새로운 ORD에 접근하거나, 종점에 도달하거나, TIME-OUT이 된 경우에 중계기로 공유 큐를 통해 정보를 전달함.
  + 2. 새로운 route_node에 접근한 경우: (공유 큐에 넣는 정보 중 type: 0이 해당 경우임.) 해당 버스의 정보를 기반으로 하나의 추론 row를 제작, 추론함수를 돌려 결과를 받고 공유 큐를 통해 전송기로 전달함. (이 과정에서 혹시 추론을 1초에 한번 돌릴 필요가 있는지, 아니면 그냥 실시간으로 돌려도 상관없는지) + 동시에 다음날 복습을 위해, 추론을 위해 돌린 row와 받은 추론값을 잘 섞어 저장용 json에 append함.
  + 3. 새로운 ORD에 접근한 경우: 해당 ORD까지의 시간정보는 더이상 변경할 필요가 없는 "확정된 시간"임을 공유 큐를 통해 전송기로 전달함. (정류장에 접근했으니 그 정류장은 예측정보가 당연히 필요 없다.)
  + 4. 종점에 접근한 경우: 당일 해당 STDID_HHMM에 대한 정보는 완성형이며, 더 이상 건들 필요가 없다는 정보를 전달하면 됨.
  + 5. timeout이 발생한 경우: 해당 STDID_HHMM에 대한 정보는 당일 더 이상 갱신되지 않을 것임을 알려야 함. (만약 종점에 도달하지 못한 부분이 있는 경우에, 사용자에게 해당 부분은 실제 도착정보 기반이 아닌 추정치임을 확실히 알려야 함.)

 - 비교 대상: 전주시버스로, 카카오버스, 전국 스마트 버스, 네이버지도, 전주버스 앱 등등...

---
### 2025-06-05
#### 진행 상황
 - 금일 미국 비자 면접으로 인해 진행을 유지하기가 힘들다.
 - 우선 26시간 걸린 학습의 결과가 나왔다. 1차 모델의 경우... batch 64가 최선인듯하다. 다만 lr과 epoch이 조금 고민이다.
 - 최초 0.001, epoch 15로 실험을 해봤을 때도 줄어드는 추세가 명확히 잘 보였는데, lr 0.0007, epoch 25는 조금 성능이 떨어진다.
 - 너무 단편적인 정보만 보고 판단하는건 아닐까 걱정된다. 조언을 구할 곳이 명확히 있으면 좋을텐데...
 - 우선은 lr = 0.001, epoch = 15로 고정해야겠다. 현재까지 최선이니.
 - 와... 일단 진짜로 꽤 잘 나온다. 좀 당황스러울 정도. 1차 모델은 전체에서 50%까지는 2분 안 오차를 갖고, 2차 모델은 ORD+1 기준 13초 오차를 갖는다..
 - 2차 모델 ORD+5 기준으로도 절반은 50초 안으로 예측한다. 75%까지 늘려도 93초. 평균이 91초이니, outlier들이 존재하는듯하다.
 - 2차 모델은 2번만 더 실험해보자. batch 512, batch 128. 둘 모두 합쳐서 4일은 잡아야하지 않을까 싶긴 하다... 하는 김에 1차도 그냥 같이 돌려버리자.
 - 1차는 batch 64 유지하고, lr은 0.0008로 세팅. epoch을 20으로 줄여봐야겠다.

#### 노트
 - 2차 모델 개선 방향: 지금 모델에서 시퀀스 개념을 도입할 수 있을지 여부에 관한 고찰.
  + 같은 ORD그룹에 속한 이전 route node가 추정한 값보다는 적게 추정해야하지 않을까.
  + 일단 정보를 5초단위로 해상도를 훨씬 늘렸으니 6월 4일부터의 학습은 lr를 조금 더 낮추는쪽으로 조정해도 괜찮지 않을까 하는 고민.

---
### 2025-06-06
#### 진행 상황
 - 1차는 batch 64, lr 0.001, epoch 15가 최선인거같다.
 - 2차는 batch 256이 512보다 더 좋았다. 128 실험으로 넘어가기 전에 shuffle을 꺼보고 실험해봐야겠다.
 - 어떻게 보면 ORD순서가 어느정도 보장되는게 더 낫다고 보기 떄문이다. overfit 걱정보단 underfit되지 않았나 걱정되는 상황이다.